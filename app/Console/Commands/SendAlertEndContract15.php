...
class SendAlertEndContract15 extends Command
{
     ...
     * @var string
     */
    protected $signature = 'send:alertendcontract15';
     ...
     * @var string
     */
    protected $description = 'the alert is sent when a contract is one month (15 days) away from ending '; //se envia la alerta cuando un contrato esta a un mes de terminar
       ...
       /*$destinatarios = Destinatario::where('is_deleted', 0)
            ...
            ->toArray();*/
             ...
             $alerta_fechas = Contrato::with('cliente')
                            ...
                            ->where('alerta_fin_contrato15',0) // no se ah enviado la alerta
                            ->where('estado',1) // el contrato esta activo
                            ->where('fecha_fin',Carbon::now()->addDays(15)->format('Y-m-d')) // fecha finalizacion es hoy + 30 dias
