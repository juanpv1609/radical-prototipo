  ...
  <table class="blueTable">
    <thead>
      <tr>
        <th>CLIENTE</th>
        <th>ENTREGABLE</th>
        <th>ESTRUCTURA DE ENTREGABLE</th>
    ...
    <tbody>
      ...
      <tr>
        <td>
          {{ $alerta->contrato->cliente->nombre_comercial }}
        </td>
        <td> {{ $alerta->descripcion }} </td>
        <td>
          @php
          $estructura = $alerta->contrato->estructura_informe;
          $estructura_entregable = explode(',', $estructura);
          @endphp

          @if (count($estructura_entregable) > 0)
          <ul>
            @foreach ($estructura_entregable as $item)
            <li>{{ trim($item) }}</li>
            @endforeach
          </ul>
          @else
          <li>Sin formato especificado</li>
          @endif

          @php
          $estructura_entregable = [];
          @endphp
