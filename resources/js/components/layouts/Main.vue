            ...
            <v-navigation-drawer app v-model="drawer" :width="220" :mini-variant.sync="mini">
                ...
                <v-list dense nav>
                    <div v-for="item in items" :key="item.title">
                        ...
                        <v-list-group v-else-if="(item.subLinks)" :key="item.title" no-action :prepend-icon="item.icon"
                    ...
                    <div v-for="item in itemsPersonal" :key="item.title">
                        <v-list-item v-if="!item.subLinks" link :to="item.link" :disabled="(item.isAdmin)"
                            color="orange darken-4">
                            <v-list-item-icon>
                                <v-icon>{{ item.icon }}</v-icon>
                            </v-list-item-icon>
                        ...
                        <v-list-group v-else-if="(item.subLinks) && (!item.isAdmin)" :key="item.title" no-action
                ...
                <v-dialog v-model="dialog" persistent max-width="400px" :loading="loading">
                    <v-form ref="form" v-model="valid" lazy-validation>
                        <v-card>
                            ...
                            <v-card-text>
                                <v-alert v-if="has_error" color="red" type="error">{{ error }}</v-alert>
                                <v-row dense>
                                    <v-col cols="12">
                                        <v-text-field v-model="form.old_password" label="Contraseña actual*" required
                                            type="password" :rules="[passwordRules2]" dense></v-text-field>
                                    </v-col>
                                </v-row>
                                <v-row dense>
                                    <v-col cols="12">
                                        <v-text-field v-model="form.new_password" label="Nueva contraseña*" required
                                            type="password" :rules="[passwordRules2]" dense></v-text-field>
                                    </v-col>
                                </v-row>
                                <v-row dense>
                                    <v-col cols="12">
                                        <v-text-field v-model="form.new_password2" label="Repita la contraseña*" required
                                            type="password"
