    ...
    <div>
        <v-card elevation="2" :loading="loading">
            <v-card-title class="d-flex justify-space-between mb-6">
                <span>Notificar plazo del envío de informe técnico</span>
            </v-card-title>
            <v-card-text>
                <v-form>
                    <v-row>
            ...
            <v-card-text>
                <v-data-table :headers="headers" :items="incidents">
                    <template v-slot:item.dateLastModified="{ item }">
                        {{ (new Date(item.dateLastModified)).toLocaleDateString('en-US') }}
                    </template>
    ...
    data() {
        return {
            ...
            headers: [
                ...
                { text: "Fecha Cierre", value: 'dateLastModified' },
    ...
    methods: {
        fetchIncidents() {
            this.$nextTick(() => {
                ...
                if (selectedPriority) {
                    ...
                    this.$nextTick(() => {
                        this.axios
                            .get(`/api/informeTecnico/${selectedPriority}`)
                            .then((response) => {
        ...
        async sendMails() {
            ...
            try {
                ...
                const response = await this.axios.get(`/api/notificarInforme/${selectedPriority}`);
